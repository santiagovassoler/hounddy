defmodule HounddyWeb.Resolvers.Profiles do
  ...
  def create_candidate(_, %{input: input}, %{context: %{current_user: current_user}}) do
    with {:ok, candidate} <- Profiles.create_candidate(input, current_user) do
      {:ok, candidate}
    end
  end

  def update_candidate(_, %{id: id, input: input}, _) do
    with {:ok, candidate} <- Profiles.update_candidate(id, input) do
      {:ok, candidate}
    end
  end

  def delete_candidate(_, %{id: id}, _) do
    with {:ok, candidate} <- Profiles.delete_candidate(id) do
      {:ok, candidate}
    end
  end
